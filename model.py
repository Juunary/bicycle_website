...
import numpy as np
from scroll import data_customerpick

#데이터 값 받기
Get_year, Get_month ,Get_date, Get_selectedCity, Get_selectedModel=data_customerpick()
...
def train_and_predict_rf(X_train, X_test, y_train, y_test):    
    # 최적의 하이퍼파라미터로 랜덤 포레스트 모델 생성 및 훈련
    rf_model = RandomForestRegressor(n_estimators=300, max_features='sqrt', max_depth=20, random_state=42)
    ...
    # 예측 수행
    predicted_CNT = rf_model.predict(result_df.values.astype(np.float32))
...
def train_and_predict_xgb(X_train, X_test, y_train, y_test):    
    # XGBoost 모델 훈련
...
if Get_selectedModel==1:
  # 함수 실행
  train_and_predict_rf(X_train, X_test, y_train, y_test)
elif Get_selectedModel == 2:
  #함수 실행
  train_and_predict_xgb(X_train, X_test, y_train, y_test)
else:
    print("Invalid model selection. Please select 1 for RandomForest or 2 for XGBoost.")